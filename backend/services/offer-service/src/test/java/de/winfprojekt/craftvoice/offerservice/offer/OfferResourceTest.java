...
class OfferResourceTest {
    ...
    void shouldUpdateStatusToAngenommenManually() {
        Offer offer = QuarkusTransaction.requiringNew().call(() -> {
            Offer o = new Offer();
            o.customerId = "1";
            o.handwerkerId = "99";
            o.businessKey = "angebot-" + UUID.randomUUID();
            o.status = Offer.STATUS_VERSENDET;
            o.persist();
            return o;
        });

        Mockito.doNothing().when(processEngineClient).sendAngebotAngenommen(offer.businessKey);
        ...
        given()
                ...
                """)
                .when()
                .put("/offers/{businessKey}/status", offer.businessKey)
                .then()
                .statusCode(200);

        QuarkusTransaction.requiringNew().run(() -> {
            Offer updatedOffer = Offer.findById(offer.id);
            assertNotNull(updatedOffer);
            assertEquals(Offer.STATUS_ANGENOMMEN, updatedOffer.status);
            assertTrue(updatedOffer.statusHistory.stream()
                    .anyMatch(h -> Offer.STATUS_ANGENOMMEN.equals(h.status)));
        });

        verify(processEngineClient, times(1)).sendAngebotAngenommen(offer.businessKey);
    }
    ...
    void shouldUpdateStatusToAbgelehntManually() {
        Offer offer = QuarkusTransaction.requiringNew().call(() -> {
            Offer o = new Offer();
            o.customerId = "1";
            o.handwerkerId = "99";
        ...
        Mockito.doNothing().when(processEngineClient).sendAngebotAbgelehnt(offer.businessKey);
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            ...
            assertEquals(Offer.STATUS_ABGELEHNT, updatedOffer.status);
            assertTrue(updatedOffer.statusHistory.stream()
                    .anyMatch(h -> Offer.STATUS_ABGELEHNT.equals(h.status)));
        });

        verify(processEngineClient, times(1)).sendAngebotAbgelehnt(offer.businessKey);
    }
    ...
    void shouldReturn409WhenUpdatingStatusOfNonVersendetOffer() {
        Offer offer = QuarkusTransaction.requiringNew().call(() -> {
            Offer o = new Offer();
            o.customerId = "1";
            o.handwerkerId = "99";
    ...
    void shouldReturn400WhenTargetStatusInvalid() {
        Offer offer = QuarkusTransaction.requiringNew().call(() -> {
            Offer o = new Offer();
            o.customerId = "1";
            o.handwerkerId = "99";
